...
async function test() {
    const { data, error } = await supabase.from('recipes').select('title, image, step_photos').order('created_at', { ascending: false }).limit(1);
    if (error) {
        ...
        console.log(`Step 1 Photo URL: ${r.step_photos ? r.step_photos[0] : 'NONE'}`);
    }
}
test();
